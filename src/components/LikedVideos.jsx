...
function LikedVideos() {
       ...
       const getLikedVideos =async()=>{
        try {
          setVideoLoading(true);
           const {data} = await axios.get("/api/v1/like/liked-videos");
          ...
          console.log("liked video error", error);
          setVideoLoading(false);
        }
       }
       getLikedVideos();
  ...
  if(isAuthenticated){
    return<> <title>Liked Videos</title> <div className=' flex flex-col h-full min-h-screen w-full bg-black text-white px-2 md:px-4 lg:px-6'>
      ...
      </div>
      {
         videos && videos.length>0 ? <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4  gap-1 items-center justify-center md:justify-start lg:justify-start   w-full '>
          {
            videos.map((video)=>{
                  return  <div className='w-full' key={video._id}> <VideoCard video={video}/></div>
            })
          }
