...
function History() {
       ...
       const getHistoryVideos =async()=>{
        try {
          setVideoLoading(true);
           const {data} = await axios.get("/api/v1/history/videos");
          ...
          console.log("History video error", error);
          setVideoLoading(false);
        }
       }
       getHistoryVideos();
  ...
  if(isAuthenticated){
    return <> <title>History</title><div className=' flex flex-col h-full min-h-screen w-full bg-black text-white px-2 md:px-4 lg:px-6'>
      ...
      </div>
      {
         videos && videos.length>0 ? <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4  gap-1 items-center justify-center md:justify-start lg:justify-start   w-full '>
          {
            videos.map((video)=>{
                  return  <div className='w-full' key={video._id}> <VideoCard history={true} historyDate={video.updatedAt} video={video.video}/></div>
            })
          }
